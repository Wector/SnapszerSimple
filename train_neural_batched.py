"""Batched Neural CFR training - Maximum GPU utilization!"""

import time
import jax

from training.neural_cfr_batched import BatchedNeuralCFRTrainer, BatchedNeuralCFRConfig
from training.evaluation import evaluate_vs_random


if __name__ == '__main__':
    # Configuration
    config = BatchedNeuralCFRConfig(
        num_iterations=100,
        games_per_batch=256,         # 256 games in parallel!
        ...
        checkpoint_dir='checkpoints/neural_batched/',
        log_dir='logs/neural_batched/'
    )

    print("="*70)
    print("BATCHED NEURAL CFR TRAINING - MAXIMUM GPU UTILIZATION")
    print("="*70)
    ...
    trainer = BatchedNeuralCFRTrainer(config, rng_key)

    print("Initializing neural network...")
    print(f"GPU available: {jax.devices()[0].platform == 'gpu'}")
    print(f"Device: {jax.devices()[0]}")
    print()
    print("NOW CHECK GPU UTILIZATION WITH: nvidia-smi")
    print("You should see 80-100% GPU usage!")
    print()
    ...
    print("\nGPU UTILIZATION:")
    print("If you ran 'nvidia-smi' during training, you should have seen")
    print("80-100% GPU utilization (vs 10% before). That's the power of batching!")
